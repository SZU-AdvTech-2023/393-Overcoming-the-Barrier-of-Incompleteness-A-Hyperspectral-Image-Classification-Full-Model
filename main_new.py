...
if __name__ == "__main__":
    ...
    argparser.add_argument("--lstm", action='store_true',default=True, help="whether to use lstm")
...
for count in range(0, Experiment_num):
    ...
    def predVisIN(indices, pred, size1, size2):
        ...
        for index, item in enumerate(x):
            if item == 0:
                y[index] = np.array([0, 0, 0])  # np.array([255, 255, 255]) / 255.
            if item == 1:
                y[index] = np.array([255, 0, 0])
            if item == 2:
                y[index] = np.array([0, 255, 0])
            if item == 3:
                y[index] = np.array([0, 0, 255])
            if item == 4:
                y[index] = np.array([0, 0, 0])
            if item == 5:
                y[index] = np.array([0, 255, 255])
            if item == 6:
                y[index] = np.array([255, 0, 255])
            if item == 7:
                y[index] = np.array([192, 192, 192])
            if item == 8:
                y[index] = np.array([128, 128, 128])
            if item == 9:
                y[index] = np.array([128, 0, 0])
            if item == 10:
                y[index] = np.array([128, 128, 0])
            if item == 11:
                y[index] = np.array([0, 128, 0])
            if item == 12:
                y[index] = np.array([128, 0, 128])
            if item == 13:
                y[index] = np.array([0, 128, 128])
            if item == 14:
                y[index] = np.array([0, 0, 128])
            if item == 15:
                y[index] = np.array([255, 165, 0])
            if item == 16:
                y[index] = np.array([255, 215, 0])
    ...
    y_pred = predVisIN(indices, y_pred_tes, r, c)
    y_pred = Image.fromarray(np.uint8(y_pred))
    # fig_path = './ksc.png'
    fig_path = os.path.join(StorageLocation + str(FLAG)+'_'+ str(count) + '.png')
    y_pred.save(fig_path, "PNG")
